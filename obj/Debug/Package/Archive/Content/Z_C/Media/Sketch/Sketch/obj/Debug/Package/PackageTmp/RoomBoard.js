   ...
   getBackColor:function(){
        ...
        updateThumbinal: function (slideid) {
            ...
            if ($('li[slide=' + slideid + ']').length == 0) {
                ...
                $('#slidemenu').append('<li  slide=' + slideid + ' onclick="boardctx.selectslide(\'' + slideid + '\');boardctx.broadcastSlideSelection(\'' + slideid + '\');"  class="small-board" ><figure><img src=' + canvas.toDataURL() + ' alt="" /></figure></li>');
        ...
        broadcastPatch: function () {
            setTimeout(function () {
                ...
                if (currentslide) {
                    var slidehstories = [];
                    var slideHistoryLength = currentslide.SlideHistories.length;
                    slidehstories.push(JSON.stringify(currentslide.SlideHistories[slideHistoryLength - 1]));

                    boardhub.server.sendPatch(room, currentslide.SlideNo, slidehstories, slideHistoryLength);
        ...
        broadcast: function (slideNo) {//initCanvas, room, SlideNo, SlideHistories, length) {
            ...
            setTimeout(function () {
                ...
                var broadcastingSlide = {};
                if (slideNo == null) {
                    broadcastingSlide = currentslide;
                }
                else {
                    broadcastingSlide = Enumerable.From(slides).First(function (x) { return x.SlideNo == slideNo });

                }
                if (broadcastingSlide) {
                    var slidehstories = [];
                    for (var i = 0; i < broadcastingSlide.SlideHistories.length; i++)
                        slidehstories.push(JSON.stringify(broadcastingSlide.SlideHistories[i]));
                  
// var lz = lzw_encode(slidehstories); // LZString.compressToUTF16(jsonstring);

 //alert(JSON.stringify( slidehstories).length+'   '+lz.length);
				  boardhub.server.sendCanvas(room, broadcastingSlide.SlideNo, slidehstories);
